...
function recalc_todo_cnt() {
    ...
    var todo_items = document.getElementsByClassName("todo_item");
    for (var i = 1; i < checkboxes.length; i++) {
        if (!checkboxes[i].checked) {
            todo_cnt++;
            todo_items[i].parentNode.className = "";
        } else {
            todo_items[i].parentNode.className = "done";
        }
    }
    ...
    if (todo_cnt != items_present) {
        if (items_present - todo_cnt < 2)
            ...
            item_txt = " items";
        cnt_div.innerHTML += "<a id=\"clear_checked\">Clear " + (items_present - todo_cnt) + " completed" + item_txt + "</a>";
